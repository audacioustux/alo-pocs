...
object CounterActor {
  def apply(next_actor: ActorRef[Int]): Behavior[Int] =
    Behaviors.setup(context => new CounterActor(context, next_actor))
}

class CounterActor(
    context: ActorContext[Int],
    next_actor: ActorRef[Int]
) extends AbstractBehavior[Int](context) {
  override def onMessage(n: Int): Behavior[Int] = {
    next_actor ! (n + 1)
    this
  }
}
...
object Main {
  def apply(n: Int): Behavior[Int] =
    Behaviors.setup(context => new Main(context, n))

  @main def init(n: Int): Unit = {
    ActorSystem(Main(n), "testSystem")
  }
}

class Main(context: ActorContext[Int], n: Int)
    extends AbstractBehavior[Int](context) {
  val time = System.currentTimeMillis()
  val last = (1 to n).foldLeft(context.self)((acc, _) =>
    context.spawnAnonymous(CounterActor(acc))
  )
  last ! 0
  override def onMessage(msg: Int): Behavior[Int] = {
