...
object BenchmarkActors {
  ...
  class PingPong(val messagesPerPair: Int, latch: CountDownLatch)
      extends Actor {
  ...
  object PingPong {
    def props(messagesPerPair: Int, latch: CountDownLatch) = Props(
      new PingPong(messagesPerPair, latch)
    )
  }
  ...
  class EchoSender(messagesPerPair: Int, latch: CountDownLatch, batchSize: Int)
      extends Actor {
    private val echo = context.actorOf(
      Props[Echo]().withDispatcher(context.props.dispatcher),
      "echo"
    )
    ...
    val actors = List
      .fill(numPairs) {
        val ping = system.actorOf(
          PingPong
            .props(messagesPerPair, latch)
            .withDispatcher(fullPathToDispatcher)
        )
  ...
  private def initiatePingPongForPairs(
      refs: Vector[(ActorRef, ActorRef)],
      inFlight: Int
  ): Unit = {
    for {
      (ping, pong) <- refs
  ...
  private def startEchoActorPairs(
    ...
    val actors = (1 to numPairs).map { _ =>
      system.actorOf(
        EchoSender
          .props(messagesPerPair, latch, batchSize)
          .withDispatcher(fullPathToDispatcher)
      )
    }.toVector
    (actors, latch)
  ...
  def benchmarkPingPongActors(
    ...
    val (actors, latch) =
      startPingPongActorPairs(numMessagesPerActorPair, numPairs, dispatcher)
  ...
  def benchmarkEchoActors(
    ...
    val numPairs = numActors / 2
    val totalNumMessages = numPairs * numMessagesPerActorPair
    val (actors, latch) = startEchoActorPairs(
