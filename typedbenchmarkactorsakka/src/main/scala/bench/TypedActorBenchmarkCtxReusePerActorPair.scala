...
object TypedActorBenchmarkCtxReusePerActorPair {
  // Constants because they are used in annotations
  final val threads = 4 // update according to cpu
  final val numMessagesPerActorPair = 35000000 // messages per actor pair
...
class TypedActorBenchmarkCtxReusePerActorPair {
  import TypedBenchmarkActorsCtxReusePerActorPair._
  ...
  def setup(): Unit = {
    ...
    system = ActorSystem(
      TypedBenchmarkActorsCtxReusePerActorPair.echoActorsSupervisor(
  ...
  def echo(): Unit = {
    Await.result(
      system.ask(TypedBenchmarkActorsCtxReusePerActorPair.Start.apply),
      timeout
    )
  }
