...
object TypedBenchmarkActorsOptionEcho {
  ...
  def echoBehavior(
      ...
      polyCtx: Option[Context],
      jsSource: Option[Source]
  ): Behavior[Message.type] = Behaviors.receive { (_, _) =>
    val _polyCtx = polyCtx.getOrElse(
      Context
        .newBuilder()
        .allowAllAccess(true)
    ...
    _polyCtx.eval(_jsSource)

    respondTo ! Message
    echoBehavior(respondTo, Some(_polyCtx), Some(_jsSource))
  }
  ...
  private def echoSender(
    ...
    Behaviors.setup { ctx =>
      val echo =
        ctx.spawn(echoBehavior(ctx.self, None, None), "echo", childProps)
    ...
    Behaviors.setup { ctx =>
      Behaviors.receiveMessage {
        case StartPingPong(
          ...
          val (actors, latch) = startPingPongActorPairs(
            ctx,
            numMessagesPerActorPair,
            numPairs,
            dispatcher
  ...
  private def initiatePingPongForPairs(
      refs: Vector[(ActorRef[Message], ActorRef[Message])],
      inFlight: Int
  ): Unit = {
    for {
      (ping, pong) <- refs
  ...
  private def startPingPongActorPairs(
    ...
    val pingPongProps =
      Props.empty.withDispatcherFromConfig(fullPathToDispatcher)
    val actors = for {
  ...
  private def newPingPongBehavior(
      messagesPerPair: Int,
    ...
    Behaviors.setup { ctx =>
      var left = messagesPerPair / 2
      val pong =
        Message(ctx.self) // we re-use a single pong to avoid alloc on each msg
