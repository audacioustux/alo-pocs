...
class TypedForkJoinActorBenchmark {
  ...
  @Setup(Level.Trial)
  def setup(): Unit = {
    BenchmarkActors.requireRightNumberOfCores(cores)
    system = ActorSystem(
  ...
  def runPingPongBench(
    ...
    val response: Future[PingPongStarted] =
      system.ask[PingPongStarted](ref =>
        StartPingPong(numMessages, numActors, dispatcher, tpt, timeout, ref)
